...
public class OrderResource {
  ...
  @Operation(
      ...
      responses =
          @ApiResponse(responseCode = "201", description = "Successfully retrieved all the orders"))
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Item(s) added to order",
            content =
                @Content(
  ...
  public void addItemToOrder(
      ...
      @RequestBody @Parameter(description = "Item(s) to be added to the order") @Valid
          List<AddItemToOrderDTO> items) {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Item(s) deleted from order",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = OrderDTO.class))),
  ...
  public ResponseEntity<Void> deleteItemFromOrder(
      @PathVariable Long id,
      @RequestBody @Validated List<@Valid RemoveItemFromOrderDTO> itemsToDelete) {
